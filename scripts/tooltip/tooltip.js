(function($) {
  ...
  jQuery.fn.tooltip = function(options) {
    ...
    this.each(function() {
      var el = $(this);
      var title = el.attr('data-h-developer-tooltip');
      if(!title) return;
      var animating = false;
      var state;
      var timer;
      el.unbind('mouseenter').mouseenter(function() {
        var delay = getDefault('delay', options, el, 1);
        clearTimeout(timer);
        timer = setTimeout(function() {
          ...
          var t         = el.attr('data-h-developer-tooltip');
          if(t) {
            title = t;
          }
          el.removeAttr('data-h-developer-tooltip');
